...
def find_whole_song_in_mix(song_chroma, mix_chroma, sr, hop_length=512):
    ...
    song_duration_secs = librosa.frames_to_time(
        song_duration_frames, sr=sr, hop_length=hop_length
    )
    end_time = start_time + song_duration_secs

    print(
        f"Match found! Estimated start: {start_time:.2f}s, Estimated end: {end_time:.2f}s"
    )
...
def refine_whole_match(
    ...
    """
    rough_start_frame = librosa.time_to_frames(
        rough_start_time, sr=sr, hop_length=hop_length
    )
    ...
    for i in range(n_chunks):
        ...
        sim = np.dot(song_chunk.flatten(), mix_chunk.flatten()) / (
            np.linalg.norm(song_chunk.flatten()) * np.linalg.norm(mix_chunk.flatten())
            + 1e-8
        )
        if sim > threshold:
    ...
    if start_chunk is not None and end_chunk is not None:
        refined_start = librosa.frames_to_time(
            rough_start_frame + start_chunk * chunk_size, sr=sr, hop_length=hop_length
        )
        refined_end = librosa.frames_to_time(
            rough_start_frame + (end_chunk + 1) * chunk_size,
            sr=sr,
...
def find_isometric_chunk_matches(
    song_chroma, mix_chroma, sr, hop_length=512, chunk_seconds=5, n_chunks=30
):
    """
    ...
    for i, chunk_start in enumerate(chunk_indices):
        ...
        # Normalize
        song_chunk_norm = (song_chunk - np.mean(song_chunk)) / (
            np.std(song_chunk) + 1e-8
        )
...
def chunk_sim(song_idx, mix_idx, song_len, mix_len, song_chroma, mix_chroma, size):
    if (
        song_idx < 0
        or mix_idx < 0
        or song_idx + size > song_len
        or mix_idx + size > mix_len
    ):
...
def refine_outer_chunks(
    ...
    chunk_size = (
        ...
        else librosa.time_to_frames(2, sr=sr, hop_length=hop_length)
    )
    ...
    while temp >= min_chunk_size:
        ...
        sim_trim_start = chunk_sim(
            song_start, mix_start, song_len, mix_len, song_chroma, mix_chroma, temp
        )
        if sim_trim_start < sim_threshold:
            song_start += temp
            mix_start += temp
        # Try to trim from end
        sim_trim_end = chunk_sim(
            song_end - temp,
            mix_end - temp,
            song_len,
            mix_len,
    ...
    while temp >= min_chunk_size:
        ...
        sim_forward = chunk_sim(
            song_end, mix_end, song_len, mix_len, song_chroma, mix_chroma, temp
        )
...
def align_core(
    ...
    trimmed_song_norm = (trimmed_song - np.mean(trimmed_song)) / (
        np.std(trimmed_song) + 1e-8
    )
    mix_segment = mix_chroma[:, mix_start:mix_end]
    mix_segment_norm = (mix_segment - np.mean(mix_segment)) / (
        np.std(mix_segment) + 1e-8
    )

    correlation = correlate(
        mix_segment_norm, trimmed_song_norm, mode="valid", method="fft"
    )
    time_correlation = np.sum(correlation, axis=0)
    ...
    trimmed_song_start = librosa.frames_to_time(
        trimmed_song_start, sr=sr, hop_length=hop_length
    )
    trimmed_song_end = librosa.frames_to_time(
        trimmed_song_end, sr=sr, hop_length=hop_length
    )
    trimmed_mix_start = librosa.frames_to_time(
        trimmed_mix_start, sr=sr, hop_length=hop_length
    )
    trimmed_mix_end = librosa.frames_to_time(
        trimmed_mix_end, sr=sr, hop_length=hop_length
    )
...
def chunk_match(
    ...
    rough_start, _ = find_whole_song_in_mix(
        song_chroma, mix_chroma, sr, hop_length=hop_length
    )
    ...
    rough_start_frame = librosa.time_to_frames(
        rough_start, sr=sr, hop_length=hop_length
    )
    # Step 3: Limit mix chroma to buffer zone
    mix_start = max(0, rough_start_frame - buffer_frames)
    mix_end = min(
        mix_chroma.shape[1], rough_start_frame + buffer_frames + song_chroma.shape[1]
    )
    ...
    for i, chunk_start, best_mix_start, best_match_score in matches:
        adjusted_matches.append(
            (i, chunk_start, best_mix_start + mix_start, best_match_score)
        )

    # Step 5: Find largest isometry block
    chunk_size = librosa.time_to_frames(chunk_seconds, sr=sr, hop_length=hop_length)
    anchor_song_start, anchor_mix_start, best_indices = find_largest_isometry(
        adjusted_matches, chunk_size
    )
    if not best_indices:
    ...
    sorted_indices = sorted(
        sorted_indices,
        key=lambda idx: (adjusted_matches[idx][1], adjusted_matches[idx][2]),
    )
    ...
    print(
        f"Partial refined match: Song Start: {librosa.frames_to_time(refined_song_start, sr=sr, hop_length=hop_length)}, Song End: {librosa.frames_to_time(refined_song_end, sr=sr, hop_length=hop_length)}"
    )
    print(
        f"    Mix Start: {librosa.frames_to_time(refined_mix_start, sr=sr, hop_length=hop_length)}, Mix End: {librosa.frames_to_time(refined_mix_end, sr=sr, hop_length=hop_length)}"
    )
    ...
    (
        ...
        min_chunk_factor=min_chunk_factor,
    )

    # Convert to seconds
    song_start_sec = librosa.frames_to_time(
        refined_song_start, sr=sr, hop_length=hop_length
    )
    song_end_sec = librosa.frames_to_time(
        refined_song_end, sr=sr, hop_length=hop_length
    )
    mix_start_sec = librosa.frames_to_time(
        refined_mix_start, sr=sr, hop_length=hop_length
    )
